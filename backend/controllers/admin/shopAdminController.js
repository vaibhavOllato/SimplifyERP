...
export const notifyNewShop = async (req, res) => {
  try {
    const newShop = await Shop.findOne({ status: 'pending' }).sort({ createdAt: -1 }); // Get the most recent pending shop
    if (!newShop) {
      return res.status(404).json({ message: 'No new shops pending verification' });
    }

    res.json({ newShop: { shopId: newShop.shopId, shopName: newShop.shopName } });
  } catch (error) {
